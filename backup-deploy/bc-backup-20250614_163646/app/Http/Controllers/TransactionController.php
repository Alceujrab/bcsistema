...
class TransactionController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Gerar hash de importação para evitar duplicatas
            $validatedData['import_hash'] = $this->generateImportHash($validatedData);
            
            // Verificar se já existe transação com o mesmo hash
            $existingTransaction = Transaction::where('import_hash', $validatedData['import_hash'])->first();
            if ($existingTransaction) {
                return back()->with('warning', 'Já existe uma transação similar cadastrada.')
                           ->withInput();
            }
            ...
            return back()->with('error', 'Erro ao criar transação: ' . $e->getMessage())->withInput();
        }
    }
