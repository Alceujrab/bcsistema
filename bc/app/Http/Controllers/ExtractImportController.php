...
class ExtractImportController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('extract-imports.create', compact('bankAccounts', 'supportedFormats', 'bankPatterns'));
    }

    public function store(Request $request)
    {
        $request->validate([
            'file' => 'required|file|max:10240|mimes:csv,txt,ofx,qif,pdf,xls,xlsx', // 10MB max, formatos específicos
        ...
        try {
            ...
            if (!$file || !$file->isValid()) {
                return redirect()->back()
                    ->withErrors(['file' => 'Arquivo inválido ou não foi enviado corretamente'])
                    ->withInput();
            }
            ...
            if (!in_array($extension, $supportedFormats)) {
                return redirect()->back()
                    ->withErrors(['file' => "Formato de arquivo não suportado: {$extension}. Formatos aceitos: " . implode(', ', $supportedFormats)])
                    ->withInput();
            }
            ...
            if (!$result['success']) {
                ...
                Log::error('Erro na importação de extrato', [
            ...
            Log::error('Exceção na importação de extrato', [
                'file' => $request->hasFile('file') ? $request->file('file')->getClientOriginalName() : 'N/A',
                'error' => $e->getMessage(),
            ...
            return redirect()->back()
                ->withErrors(['file' => 'Erro interno ao processar arquivo. Verifique se o formato está correto e tente novamente. Erro: ' . $e->getMessage()])
                ->withInput();
        }
    }
    ...
    protected function saveTransactions($transactions, $bankAccountId, $options)
    {
        $created = 0;
        $skipped = 0;
        $importHash = 'import_' . time() . '_' . $bankAccountId;
        ...
        foreach ($transactions as $transactionData) {
            ...
            // Gerar hash único para esta transação
            $transactionHash = $importHash . '_' . $created;

            // Criar transação com os campos corretos da migration
            Transaction::create([
                ...
                'status' => 'pending', // Para permitir conciliação posterior
    ...
    protected function logImport($data)
    {
        ImportLog::create([
            ...
            'imported_records' => $data['imported_records'],
    ...
    {
        return ImportLog::with('bankAccount')
            ->latest()
            ->limit($limit)
            ->get();
    }
    ...
    {
        return ImportLog::with('bankAccount')->findOrFail($id);
    }
