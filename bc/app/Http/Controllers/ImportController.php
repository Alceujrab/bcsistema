...
class ImportController extends Controller
{
        ...
        $request->validate([
            ...
            'file' => [
                ...
                function ($attribute, $value, $fail) {
                    if (!$this->validateFileFormat($value)) {
                        $fail('Formato de arquivo não suportado. Use: CSV, TXT, OFX, QIF, PDF, XLS ou XLSX.');
                    }
                }
            ],
            'import_type' => 'required|in:bank_statement,manual,api'
        ], [
            ...
            'import_type.required' => 'Tipo de importação é obrigatório',
    ...
    {
        $allowedExtensions = ['csv', 'txt', 'ofx', 'qif', 'pdf', 'xls', 'xlsx'];
    ...
    /**
     * Validar se o PDF é um extrato válido
     */
    private function validatePdfFormat($file)
    {
        if (strtolower($file->getClientOriginalExtension()) !== 'pdf') {
            return true; // Não é PDF, pular validação
        }
        ...
        try {
            ...
            // Verificar assinatura PDF básica
