...
class ReconciliationController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        return view('reconciliations.create', compact(
            ...
            'pendingTransactionsList' => $pendingTransactions,
    ...
    public function store(Request $request)
    {
        ...
        $validated['created_by'] = 1; // TODO: Implementar autenticação adequada
    ...
    public function approve(Reconciliation $reconciliation)
    {
        ...
        $reconciliation->update([
            'status' => 'approved',
            'approved_by' => 1, // TODO: Implementar autenticação
            'approved_at' => now(),
        ]);
    ...
    public function transferTransactionDuringReconciliation(Request $request)
    {
        ...
        try {
            ...
            // Log da transferência seria registrado aqui
    ...
    /**
     * Obter transações disponíveis para conciliação
     */
    public function getAvailableTransactions(Request $request)
    {
        $bankAccountId = $request->input('bank_account_id');
        ...
        if (!$bankAccountId || !$startDate || !$endDate) {
            return response()->json(['error' => 'Parâmetros obrigatórios não fornecidos'], 400);
        }

        $transactions = Transaction::where('bank_account_id', $bankAccountId)
            ->whereBetween('transaction_date', [$startDate, $endDate])
            ->where(function($query) {
                $query->whereNull('reconciliation_id')
        ...
        // Importações relacionadas ao período
        $imports = StatementImport::where('bank_account_id', $bankAccountId)
            ->where('status', 'completed')
            ->whereBetween('created_at', [$startDate, $endDate])
            ->get(['id', 'filename', 'created_at', 'imported_transactions as imported_records']);
        ...
        return response()->json([
            'transactions' => $transactions->map(function($transaction) {
                return [
                    ...
                    'can_reconcile' => is_null($transaction->reconciliation_id)
                ];
            }),
            'imports' => $imports,
            'summary' => [
