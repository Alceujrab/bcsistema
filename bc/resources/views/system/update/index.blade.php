                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            ...
                            @if(version_compare($update['version'], $systemInfo['current_version']) > 0)
                        ...
                        <td>
                            @if(isset($update['file_size']) && $update['file_size'])
                                @php
                        ...
                        <td>
                            <div class="btn-group btn-group-sm">
                                <button class="btn btn-outline-info" 
                                        onclick="showUpdateDetails('{{ $update['id'] }}', '{{ $update['name'] ?? 'Sem nome' }}', '{{ $update['version'] }}', '{{ $update['description'] ?? 'Sem descrição' }}')">
...
function createBackup() {
    ...
    fetch('{{ route("system.update.backup.create") }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        }
...
function showUpdateDetails(id, name, version, description) {
    const modal = `
        <div class="modal fade" id="updateDetailsModal" tabindex="-1">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-success" onclick="validateUpdate('${id}')">
    ...
    // Mostra o modal
    const modalElement = new bootstrap.Modal(document.getElementById('updateDetailsModal'));
    modalElement.show();
